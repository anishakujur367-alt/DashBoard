...
export default function DashboardClient({
  ...
  // Build a merged UserProfile: live DB values take priority, mock values as fallback
  const liveUser = {
    ...mockUser,
    name: initialUser?.name ?? mockUser.name,
    avatar: initialUser?.avatar ?? mockUser.avatar,
    level: initialUser?.level ?? mockUser.level,
    currentXP: initialUser?.current_xp ?? mockUser.currentXP,
    nextLevelXP: initialUser?.next_level_xp ?? mockUser.nextLevelXP,
    streakDays: initialUser?.streak_days ?? mockUser.streakDays,
    streakActive: initialUser?.streak_active ?? mockUser.streakActive,
    totalStudyHours: initialUser?.total_study_hours ?? mockUser.totalStudyHours,
    completedCourses: initialUser?.completed_courses ?? mockUser.completedCourses,
  };

  // Use live deadlines if available, else fall back to empty (BentoGrid/DeadlinesTile handles empty state)
  const deadlines: Deadline[] = initialDeadlines.length > 0 ? initialDeadlines : [];
  ...
  // Sync state if initialStudents changes from SSR
  useEffect(() => {
    setStudents(initialStudents);
  }, [initialStudents]);

  // Enable Real-Time Listener on database changes
  useEffect(() => {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "INSERT") {
            const record = payload.new as Student;
            ...
            setStudents((prev) => prev.filter((item) => item.id !== record.id));
          }
        }
      )
      .subscribe();
      ...
      interval = setInterval(() => {
        setTimeLeft((prev) => {
          if (prev <= 1) {
            ...
            // Delay the alert slightly so the UI updates to 00:00 first
            setTimeout(() => {
      ...
      <div className="flex-1 flex flex-col min-w-0 max-h-screen overflow-y-auto">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:block">
              <h2 className="text-sm font-black text-text-secondary uppercase tracking-widest flex items-center gap-2">
                ...
                <span className="text-[10px] bg-slate-base border border-card-border text-text-secondary px-2 py-0.5 rounded font-bold">v1.3.0</span>
              </h2>
            </div>
          </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === "dashboard" ? (
            ...
            >
              <BentoGrid user={liveUser} students={students} isError={isError} deadlines={deadlines} />
            </motion.div>
          ) : (
            <motion.div
              key="other-views"
            ...
            >
              <div className="max-w-md glass-panel glass-panel-glow p-8 rounded-3xl relative overflow-hidden flex flex-col items-center">
                ...
                <h3 className="text-xl font-black text-text-primary tracking-tight">
                  {liveUser.name}&apos;s {activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}
                </h3>
